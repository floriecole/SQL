...
-- 1b. Display the first and last name of each actor in a single column in upper case letters. Name the column `ActorName`
...
SELECT f.title, COUNT(rental_id) AS 'Count of Rented Movies'
FROM rental r
JOIN inventory i
...
GROUP BY f.title
ORDER BY `Count of Rented Movies` DESC;

-- 7f. Write a query to display how much business, in dollars, each store brought in.
SELECT s.store_id, SUM(amount) AS 'Total Amount Made'
FROM payment p
JOIN rental r
...
SELECT c.name AS "Top_Five_Genre", SUM(p.amount) AS "Gross" 
FROM category c
JOIN film_category fc ON (c.category_id=fc.category_id)
JOIN inventory i ON (fc.film_id=i.film_id)
JOIN rental r ON (i.inventory_id=r.inventory_id)
